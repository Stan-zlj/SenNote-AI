...
const NoteView: React.FC<NoteViewProps> = ({ 
  ...
  const [hasClipboardUpdate, setHasClipboardUpdate] = useState(false);
  ...
  // "Save what I copy" helper: Detect clipboard on focus
  useEffect(() => {
    const checkClipboard = async () => {
      try {
        const text = await navigator.clipboard.readText();
        const lastNoteContent = notes[0]?.content;
        if (text && text !== lastNoteContent && text !== inputValue) {
          setHasClipboardUpdate(true);
        } else {
          setHasClipboardUpdate(false);
        }
      } catch (e) { /* Permission denied or not focused */ }
    };
  ...
  const handlePasteFromClipboard = async () => {
    try {
      ...
      alert("Please allow clipboard access permissions in your browser settings.");
    }
  };
    ...
    <div className="space-y-6">
      <header className="flex justify-between items-end">
        <div>
          ...
          <p className="text-slate-400 text-sm">Smartly capture and analyze your study fragments.</p>
        </div>
        <div className="flex items-center gap-4">
          ...
          <button 
            ...
            className={`px-4 py-2 rounded-xl text-xs font-bold transition-all flex items-center gap-2 border shadow-lg ${hasClipboardUpdate ? 'bg-indigo-600 border-indigo-400 text-white animate-pulse' : 'bg-slate-800/50 border-white/5 text-slate-400 hover:text-white'}`}
          >
            {hasClipboardUpdate ? 'New Clipboard Found!' : 'Paste from Clipboard'}
          </button>
        </div>
      </header>

      <form onSubmit={handleSubmit} className="bg-slate-800/20 border border-white/5 p-4 rounded-2xl space-y-3 shadow-inner">
        <textarea value={inputValue} onChange={(e) => setInputValue(e.target.value)} placeholder="Type or paste content here..." className="w-full h-24 bg-slate-900/40 border border-white/10 rounded-xl p-4 text-slate-200 focus:outline-none focus:ring-2 focus:ring-indigo-500/30 transition-all resize-none text-sm"/>
        <div className="flex gap-3">
          <input type="text" value={tagInput} onChange={(e) => setTagInput(e.target.value)} placeholder="Tags (comma separated)..." className="flex-1 px-4 py-2 bg-slate-900/40 border border-white/10 rounded-xl text-xs text-slate-200 focus:outline-none"/>
          <button type="submit" disabled={!inputValue.trim()} className="bg-indigo-600 hover:bg-indigo-700 text-white px-6 py-2 rounded-xl text-xs font-bold transition-all disabled:opacity-50">Save Note</button>
        </div>
      </form>
